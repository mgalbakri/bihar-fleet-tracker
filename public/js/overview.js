...
function renderOverviewThreatFeed(incidents) {
  ...
  // Sort: official sources first, then by severity, then by time
  ...
  el.textContent = '';
  sorted.slice(0, 15).forEach(function(inc) {
    ...
    loc.textContent = (inc.corridor || '') + (inc.lat ? ' \u00b7 ' + Number(inc.lat).toFixed(2) + '\u00b0N ' + Number(inc.lon).toFixed(2) + '\u00b0E' : '');
